...
def generate_for_prompts(
    ...
    for i, off in enumerate(range(0, len(prompts), batch_size)):
        ...
        with torch.no_grad():
            ...
            #likelihood = torch.cumprod(loss_per_token, dim=-1)[:,-1]
            
            loss_per_token_np = loss_per_token.numpy()
            mean = np.mean(loss_per_token_np, axis=-1)  #0
            std = np.std(loss_per_token_np, axis=-1)    #0
            ...
            for i, val in enumerate(loss_per_token_np):
                loss_per_token_np[i] = float(np.where(((val<floor[i])|(val>upper[i])), mean[i], val))
